...
> ローカル作業では `npm install` 等で構築済みであることを前提としています。

テンプレート側で用意している GitHub Actions 処理では `.node-version` が存在する前提になっている点に注意してください。

> [!WARNING]
> `.package-lock.json` が存在する前提のワークフローになっている点は注意してください。
...
`GeneratePluginConfigAttribute` を適用したクラスには、静的メソッド `Bind` とインスタンスメソッド `Reset`, `CopyTo` が生えます。
...
`CopyTo` は引数に対して呼び出し元オブジェクトの値を設定します。  
※あまり使うことはないと思いますが、ある Mod を作った時に必要になって実装して逆輸入されました。
